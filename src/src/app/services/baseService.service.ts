...
export class BaseService<EntityType extends BaseEntity> {
  ...
  getPageResult(pageSize: number = 10, currentPage: number = 1, search: string = '', orderBy: string = ''){
    ...
    return this.http.get<PagingResult<EntityType>>(this.apiUrl + '/details',{params}).pipe(tap( x => console.log(x)));
  }
